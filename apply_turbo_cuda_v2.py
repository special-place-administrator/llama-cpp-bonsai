...
if not os.path.isfile(f"{CUDA}/set-rows.cu"):
    print("ERROR: Run from the llama-cpp-rotorquant repo root."); sys.exit(1)
if 'rq-quant-cuda.cuh' in read(f"{CUDA}/set-rows.cu"):
    print("Patch already applied. Aborting."); sys.exit(1)

print("=== RotorQuant CUDA Patch v2 (turbo3 + turbo4 + mixed K/V) ===\n")
...
write(f"{CUDA}/rq-quant-cuda.cuh", r'''#pragma once
...
// === RQ4: 3-bit unpack helper ===
static __device__ __forceinline__
uint8_t rq4_unpack_3bit(const uint8_t * qs, int j) {
...
void quantize_f32_rq4_0_block(const float * src, block_rq4_0 * dst) {
    ...
    float recon[128];
    for (int j = 0; j < 128; j++) {
        uint8_t idx = rq_find_nearest_3bit(x[j]);
        recon[j] = d_rq_centroids_3bit[idx];
        int bit_offset = j * 3, byte_idx = bit_offset / 8, bit_pos = bit_offset % 8;
...
void dequantize_rq4_0(const void * vx, const int64_t ib, const int iqs, float2 & v) {
    ...
    { const int j = iqs;
      uint8_t idx = rq4_unpack_3bit(x[ib].qs, j);
      float s = (x[ib].signs[j/8] & (1 << (j%8))) ? 1.0f : -1.0f;
      v.x = (d_rq_centroids_3bit[idx] + s * qjl_scale) * norm; }
    { const int j = iqs + 64;
      uint8_t idx = rq4_unpack_3bit(x[ib].qs, j);
      float s = (x[ib].signs[j/8] & (1 << (j%8))) ? 1.0f : -1.0f;
      v.y = (d_rq_centroids_3bit[idx] + s * qjl_scale) * norm; }
}
''')
print("  Created: rq-quant-cuda.cuh")

# Template instances: turbo3×turbo3, turbo4×turbo4, plus mixed types
for kt, vt in [("rq3_0","rq3_0"), ("rq4_0","rq4_0"),
               ("rq3_0","q8_0"), ("rq4_0","q8_0"),
               ("q8_0","rq3_0"), ("q8_0","rq4_0")]:
...
insert_after(f, '#include "cpy-utils.cuh"', '#include "rq-quant-cuda.cuh"', "add include")
...
print("\n[2/7] getrows.cu")
f = f"{CUDA}/getrows.cu"
insert_after(f, '#include "convert.cuh"', '#include "rq-quant-cuda.cuh"', "add include")
...
content = content.replace(
    'GGML_TYPE_IQ4_NL) &&',
    'GGML_TYPE_IQ4_NL ||\n                       op->type == GGML_TYPE_RQ3_0 || op->type == GGML_TYPE_RQ4_0) &&', 1)
    ...
    if 'src[0]->type' in ctx or 'src0->type' in ctx:
        for i in range(loc, min(loc+20, len(lines))):
            if 'GGML_TYPE_Q8_0' in lines[i] and 'case' in lines[i]:
                lines.insert(i+1, '                    case GGML_TYPE_RQ3_0:')
                lines.insert(i+2, '                    case GGML_TYPE_RQ4_0:')
...
VEC_DOT_ALL = '''
static __device__ __forceinline__
uint8_t rq4_unpack_3bit_fattn(const uint8_t * qs, int j) {
    int bit_offset = j * 3, byte_idx = bit_offset / 8, bit_pos = bit_offset % 8;
    uint16_t raw = (uint16_t)qs[byte_idx];
    if (byte_idx + 1 < 48) raw |= (uint16_t)qs[byte_idx + 1] << 8;
    return (uint8_t)((raw >> bit_pos) & 0x7);
...
#pragma unroll
        for (int k_KQ_1 = 0; k_KQ_1 < cpy_ne; ++k_KQ_1) {
            ...
            const int ib = elem / QK_RQ3, j0 = elem % QK_RQ3;
            ...
            { const int j = j0;
              ...
              k0 = d_rq_centroids_3bit_fattn[low2 | (hi1 << 2)] * norm; }
            { const int j = j0 + 1;
              const uint8_t low2 = (K_t3[ib].qs[j/4] >> ((j%4)*2)) & 0x3;
              const uint8_t hi1  = (K_t3[ib].signs[j/8] >> (j%8)) & 0x1;
              k1 = d_rq_centroids_3bit_fattn[low2 | (hi1 << 2)] * norm; }
...
#pragma unroll
        for (int k_KQ_1 = 0; k_KQ_1 < cpy_ne; ++k_KQ_1) {
            const int elem = (base_f2 + k_KQ_1) * 2;
            const int ib = elem / QK_RQ4, j0 = elem % QK_RQ4;
            if (ib != prev_ib) {
            ...
            float k0, k1;
            { const int j = j0;
              float c = d_rq_centroids_3bit_fattn[rq4_unpack_3bit_fattn(K_t4[ib].qs, j)];
...
#pragma unroll
    for (int l = 0; l < ne; l++) {
        ...
        const uint8_t low2 = (x[ib].qs[j/4] >> ((j%4)*2)) & 0x3;
        const uint8_t hi1  = (x[ib].signs[j/8] >> (j%8)) & 0x1;
        vals[l] = d_rq_centroids_3bit_fattn[low2 | (hi1 << 2)] * norm;
    }
...
#pragma unroll
    for (int l = 0; l < ne; l++) {
        const int j = j0 + l;
        float c = d_rq_centroids_3bit_fattn[rq4_unpack_3bit_fattn(x[ib].qs, j)];
        float s = (x[ib].signs[j/8] & (1 << (j%8))) ? 1.0f : -1.0f;
...
content = content.replace(
    ...
    '(type_K == GGML_TYPE_F16 || type_K == GGML_TYPE_RQ3_0 || type_K == GGML_TYPE_RQ4_0) ? 128 / cpy_nb : nthreads_KQ_q')
content = content.replace(
    'type_V == GGML_TYPE_F16 ? 128 / cpy_nb : nthreads_V_q',
    '(type_V == GGML_TYPE_F16 || type_V == GGML_TYPE_RQ3_0 || type_V == GGML_TYPE_RQ4_0) ? 128 / cpy_nb : nthreads_V_q')
content = content.replace(
    'type_V == GGML_TYPE_F16 ? 2*cpy_ne : 4',
    '(type_V == GGML_TYPE_F16 || type_V == GGML_TYPE_RQ3_0 || type_V == GGML_TYPE_RQ4_0) ? 2*cpy_ne : 4')
content = content.replace(
    'type_K != GGML_TYPE_F16;',
    'type_K != GGML_TYPE_F16 && type_K != GGML_TYPE_RQ3_0 && type_K != GGML_TYPE_RQ4_0;')
...
# Force VEC for rq types
content = read(f); lines = content.split('\n')
for i, l in enumerate(lines):
    if 'can_use_vector_kernel' in l and 'const bool' in l:
        ins = ['',
            '    // RotorQuant: only the vec kernel has rq dequant support.',
            '    if (K->type == GGML_TYPE_RQ3_0 || V->type == GGML_TYPE_RQ3_0 ||',
...
turbo_lines = (
    '\n    FATTN_VEC_CASES_ALL_D(GGML_TYPE_RQ3_0, GGML_TYPE_RQ3_0)'
    '\n    FATTN_VEC_CASES_ALL_D(GGML_TYPE_RQ4_0, GGML_TYPE_RQ4_0)'
    '\n    FATTN_VEC_CASES_ALL_D(GGML_TYPE_RQ3_0, GGML_TYPE_Q8_0)'
    '\n    FATTN_VEC_CASES_ALL_D(GGML_TYPE_RQ4_0, GGML_TYPE_Q8_0)'
    '\n    FATTN_VEC_CASES_ALL_D(GGML_TYPE_Q8_0,     GGML_TYPE_RQ3_0)'
    '\n    FATTN_VEC_CASES_ALL_D(GGML_TYPE_Q8_0,     GGML_TYPE_RQ4_0)')
...
new_lines = [
    '        file(GLOB   SRCS "template-instances/fattn-vec*rq3_0*.cu")',
    '        list(APPEND GGML_SOURCES_CUDA ${SRCS})',
    '        file(GLOB   SRCS "template-instances/fattn-vec*rq4_0*.cu")',
    '        list(APPEND GGML_SOURCES_CUDA ${SRCS})',
]
...
print("\nTest:")
print("  # rq3 K+V (3.25 bits, 4.9x compression)")
print("  ./build/bin/llama-server -m model.gguf -ngl 99 --cache-type-k rq3 --cache-type-v turbo3 -fa")
print("  # rq4 K+V (4.25 bits, 3.8x compression, QJL correction)")
...
print("  ./build/bin/llama-server -m model.gguf -ngl 99 --cache-type-k rq3 --cache-type-v q8_0 -fa")
